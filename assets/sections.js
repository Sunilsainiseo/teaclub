/******/ (() => { // webpackBootstrap/* eslint-disable */
/* ==================================================
#Logo list
#Featured collection
#Featured promotions
...
window.cart = {
  init() {
    if ($('#cart_form .tos_agree').length) {
      ...
      $('body').on(w3elem, "#cart_form input[type='submit']", function () {
...
```
      $thumbnailProductGallery.find('.gallery-cell').on(w3elem, function () {
```        const index = $(this).index();
    ...
    function loadDynamicProducts() {
      ...
      $.ajax({
        ...
        success(data) {
          ...
          $('.swatch_options label').on(w3elem, function() {
  ...
  productSwatches() {
    ...
    if (window.PXUTheme.theme_settings.product_form_style === 'radio') {
      ...
      if ($('.js-product-section').length > 1) {
        $('body').on(w3elem, '.swatch-element', swatchElement => {
  ...
  initializeQuantityBox() {
    $('body').on(w3elem, '.js-change-quantity', e => {
  ...
  init() {
    ...
    if ($('.promo-banner').length) {
      ...
      if (promoBanner !== 'dismiss') {
        ...
        $('.promo-banner').on(w3elem, '.promo-banner__close', () => {
    ...
    if (window.PXUTheme.media_queries.large.matches) {
      if ($('.header').hasClass('header-fixed--true')){
        ...
        $('body').on(w3elem, '.banner a[href^="#"]', function(e) {
            e.preventDefault();
    ...
    $('.cart_content__continue-shopping').on(w3elem, () => {
      closeMiniCart();
    });
  },
  unload() {
  ...
  init() {
    $('[data-shoppable-image-wrapper]').each((_, wrapper) => {
      ...
      */
      ...
      document.body.addEventListener(w3elem, (e) => {
